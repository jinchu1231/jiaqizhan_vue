              ...
              <el-table-column prop="value" label="值" align="center">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.defaultValue" placeholder="请输入" clearable
                    :disabled="scope.row.useSearch" />
                </template>
              </el-table-column>
              <el-table-column prop="useSearch" label="作为查询字段" width="100">
                <template slot-scope="scope">
                  <el-checkbox v-model="scope.row.useSearch"
                    @change="onUseSearchChange($event, scope.row)"></el-checkbox>
                </template>
              </el-table-column>
            </el-table>
          </el-form>
